...
Comparaison des fonctions sur l'ensemble du dispositif BCI ($f_{>0}=225$, $s_{1}=19$):
...
Des tests poussés ont été menés par @Brose2003 pour permettre le choix du meilleur estimateur de la richesse en fonction de la complétude de l'échantillonnage $f_{>0}/{S}$. 
...
$\hat{S}^{n}_{0}$ est obtenu par la différence entre les nombres d'espèces estimé et observé: $\hat{S}^{n}_{0}=\hat{S}-f_{>0}$.
...
\begin{equation} 
  (\#eq:BiaisSimpson)
  \tilde{E} = \left(\frac{n}{n-1}\right)\left(1-\sum^{f_{>0}}_{s=1}{{\hat{p}}^2_s}\right).
\end{equation}
...
\begin{equation}
  (\#eq:ChaoShen)
  \tilde{H} = -\sum_{s=1}^{f_{>0}}{\frac{\hat{C}{\hat{p}}_s \ln\left(\hat{C}{\hat{p}}_s\right)}{1-\left(1-\hat{C}{\hat{p}}_s\right)^n}}.
\end{equation}
...
\begin{equation}
  (\#eq:Hzetanu)
  H_z = \sum^{n-1}_{v=1}{\frac{1}{v}\left\{\frac{n^{v+1}\left[n-\left(v+1\right)\right]!}{n!}\sum^{f_{>0}}_{s=1}{p_s\prod^{v-1}_{j=0}{\left(1-{\hat{p}}_s-\frac{j}{n}\right)}}\right\}}.
\end{equation}
...
\begin{equation}
  (\#eq:JamesStein2)
  \hat{\lambda} = \frac{1-\sum^{f_{>0}}_{s=1}{\left( \hat{p}_s \right)^2}}
...
\begin{equation}
  (\#eq:Zhang2014hq)
  \tilde{h}_q = 1+\sum^{f_{>0}}_{s=1}{\frac{n_s}{n}\sum^{n-n_s}_{v=1}{\left[\prod^v_{i=1}{\frac{i-q}{i}}\right]\left[\prod^v_{j=1}{\left(1-\frac{n_s-1}{n-j}\right)}\right]}}.
\end{equation}
...
\begin{equation}
  (\#eq:H1Zhang)
  ^1\!{\tilde{H}} =\sum^{f_{>0}}_{s=1}{\frac{n_s}{n}\sum^{n-n_s}_{v=1}{\frac{1}{v}\left[\prod^v_{j=1}{1-\frac{n_s-1}{n-j}}\right]}}.
\end{equation}
...
Alors, $(1-p_s)^m$ est estimé par $(1-\hat{\alpha}_0)^m$ et peut sortir de la somme qui ne contient plus que le terme $\sum_s{p_s}\mathbf{1}(n_s=0)$, estimé par $\hat{\alpha}_0 {f_{0}} = \frac{f_{1}}{n}(1 - \hat{\alpha}_1)$.
...
\begin{align} 
  ...
  &= 1-\frac{f_{1}}{n} 
    \left[ \frac{\left(n-1\right)f_{1}}{\left(n-1\right)f_{1}+2f_{2}} \right]^{n+m} \\
  &= 1-\frac{f_{1}}{n} \left( 1-A \right)^{m+1}.
\end{align}
...
\begin{equation}
  (\#eq:HqChaoShen)
  ^q\!{\tilde{H}} =-\sum^{f_{>0}}_{s=1}{\frac{\hat{C}{\hat{p}}_s \ln_q\left(\hat{C}{\hat{p}}_s\right)}{1-{\left(1-\hat{C}{\hat{p}}_s\right)}^n}}.
\end{equation}
...
\begin{equation}
  (\#eq:Grassberger)
  ^q\!{\tilde{H}}
  = \frac{1-\sum^{f_{>0}}_{s=1}{\widetilde{p^q_s}}}{q-1},
\end{equation}
...
\begin{equation}
  (\#eq:Chao2013lambda)
  \hat{\lambda}
  = \frac{1-\hat{C}}{\sum_{s=1}^{f_{>0}} {\frac{n_s}{n} \left(1-\frac{n_s}{n}\right)^n}}.
\end{equation}
