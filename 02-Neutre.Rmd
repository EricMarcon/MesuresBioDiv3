...
# Des indices de diversité aux nombres de Hill {#chap-MesuresNeutres}
...
L'entropie HCDT permet d'unifier les indices classiques de diversité : son paramètre, appelé ordre, fixe l'importance donnée aux espèces rares. 
L'entropie d'ordre 0 est la richesse; celle d'ordre 1, l'indice de Shannon; celle d'ordre 2, celui de Simpson.
...
C'est la mesure conceptuellement la plus simple mais pratiquement la plus délicate dans des systèmes très riches comme les forêts tropicales: même avec des efforts d'inventaire considérables, il n'est en général pas possible de relever toutes les espèces rares, ce qui implique de recourir à des modèles mathématiques pour en estimer le nombre, détaillés en section \@ref(#sec-est-Richesse).

La détermination des genres est plus facile et fiable que celle des espèces, le biais d'échantillonnage moins sensible (il est plus facile d'observer au moins un individu d'un genre que de chacune de ses espèces), et les coûts d'inventaire sont généralement largement réduits [@Balmford1996b].
...
@Cartozo2008 ont montré que le nombre de taxons de niveau supérieur (genre par rapport aux espèces, ordres par rapport aux sous-ordres) est universellement proportionnel au nombre de taxons du niveau immédiatement inférieur à la puissance 0,61. 
Cette relation est validée à l'échelle mondiale pour les systèmes végétaux. 
La loi de puissance reste valide pour des assemblages aléatoires, c'est donc la conséquence de propriétés mathématiques [@Caldarelli2002], mais la puissance de la relation est plus élevée (les communautés réelles sont plus agrégées du point de vue phylogénétique que sous l'hypothèse nulle d'un assemblage aléatoire) et varie entre les niveaux. 
...
Il est compris dans l'intervalle $[0 ; 1[$. 
...
Les textes fondateurs sont @Davis1941 et surtout @Theil1967 en économétrie, et Shannon [-@Shannon1948; -@Shannon1963] pour la théorie de l'information. 
Une revue est fournie par @Maasoumi1993.

Considérons une expérience dont les résultats possibles sont $\{y_1, y_2, \dots , \ y_S\}$. 
La probabilité d'obtenir $y_s$ est $p_s$, et $\mathbf{p}=(p_1, p_2, \dots, p_S)$ est le vecteur composé des probabilités d'obtenir chaque résultat. 
Les probabilités sont connues *a priori*. 
Tout ce qui suit est vrai aussi pour des valeurs de $y$ continues, dont on connaîtrait la densité de probabilité.

On considère maintenant un échantillon de valeurs de $y$. 
La présence de $y_s$ dans l'échantillon est peu étonnante si $p_s$ est grande: elle apporte peu d'information supplémentaire par rapport à la simple connaissance des probabilités. 
En revanche, si $p_s$ est petite, la présence de $y_s$ est surprenante. 
...
@Patil1982 appellent l'information "rareté", mais nous ne les suivrons pas.
...
Si le résultat est certain (une seule valeur $p_1$ vaut 1), l'entropie est nulle. 
...
### Approche axiomatique {#sec-Axiomes}
...
Les premiers travaux consistant à généraliser l'indice de Shannon pour mesurer la diversité sont dus à @Renyi1961. 
...
* L'introduction d'une espèce dans une communauté augmente sa diversité (conséquence de la décroissance de $I(p_s)$);
...
La maximisation de l'entropie est atteinte quand aucun remplacement n'est plus possible, c'est-à-dire quand toutes les probabilités sont égales.

Ces axiomes sont donc tous validés par des fonctions d'information décroissantes et concaves: dans ce cadre, il toute entropie est une mesure de diversité acceptable.
...
Appliqués à l'entropie HCDT, les nombres de Hill en sont l'exponentielle déformée du même ordre.
...
```{r}
#| label: MoyenneHardyFig
#| echo: false
#| results: 'hide'
#| ref.label: 'MoyenneHardyCode'
#| fig.cap: "(ref:MoyenneHardyFig)"
```
...
Les nombres de Hill peuvent être réécrits [@Leinster2012] sous la forme d'une moyenne généralisée:

\begin{equation}
  (\#eq:HillHardy)
  ^{q}\!D = {\left(\sum^S_{s=1}{p_s}\left(\frac{1}{p_s}\right)^{1 - q}\right)}^{\frac{1}{1-q}}.
\end{equation}

La diversité est donc la moyenne généralisée d'ordre $1 - q$ de la rareté des espèces.
Quand $q$ est grand, $1-q$ est petit: la moyenne généralisée surpondère les espèces les moins rares.

Cette définition ne repose pas sur l'entropie.

Une application immédiate de cette définition est que la diversité d'une communauté d'espèces équiprobables est égale à leur rareté: $S$ espèces de probabilité $1 / S$, donc de rareté égale à $S$, forment une communauté de diversité égale à $S$, quel que soit l'ordre considéré, conformément à la définition des nombres effectifs.


## Synthèse : entropie et diversité 

La rareté d'une espèce est définie par $1/p_s$.
Le logarithme (déformé) de la rareté définit l'information.
...
La diversité (d'ordre $q$) est aussi la rareté moyenne (d'ordre $1 - q$).
...
* $^{-\infty}\!D = 1 / \min(\mathbf{p})$ est l'inverse de la proportion de la communauté représentée par l'espèce la plus rare (toutes les autres espèces sont ignorées).
...
(ref:ProfilDivFig) Profils de diversité des quatre carrés de la parcelle 6 de Paracou. Le carré 2 est plus divers que le carré 4, mais pas que les carrés 1 et 3, qui sont plus divers dans les grands ordres de diversité. Le profil de diversité est tracé ici jusqu'à l'ordre $q = 3$.
```{r}
#| label: ProfilDivFig
#| echo: false
#| results: 'hide'
...
# Méthodes pour l'estimation de la diversité

::: {.Summary data-latex=""}
TODO
:::

Ce chapitre présente les techniques d'estimation de la diversité asymptotique: c'est une revue de la littérature méthodologique.
Une grande part de la littérature est consacrée à l'estimation de la richesse parce que c'est la mesure la plus simple et la plus utilisée. 
Dans une moindre mesure, l'estimation de l'entropie de Shannon a été abondamment étudiée, notamment par la physique statistique.
Celle de l'entropie de Simpson est anecdotique parce qu'elle est très peu sensible au biais d'estimation: par construction, plus l'ordre de diversité est grand, plus l'importance des espèces rares (responsables du biais) est faible.
Enfin, l'estimation de l'entropie HCDT en général a connu de grands progrès dans la décennie 2010.

La dernière section du chapitre introduit les méthodes d'estimation de la diversité non-asymptotique, mais à un niveau d'échantillonnage choisi, inférieur à celui des données (on parle alors de raréfaction) ou supérieur (on parle alors d'extrapolation).


## Richesse {#sec-est-richesse}

L'estimation de la richesse d'une communauté par son échantillonnage pose de grandes difficultés pratiques: dans un système hyperdivers comme une forêt tropicale humide, beaucoup d'espèces sont rares et nécessitent un effort d'échantillonnage démesuré pour être observées.
La courbe d'accumulation des espèces (SAC) n'atteint en pratique jamais son asymptote, à l'image d'un carré de 1,65ha de la parcelle 6 de Paracou (figure \@ref(fig:SACP6Fig)).

(ref:SACFig) Courbe d'accumulation des espèces d'arbres du carré 1 de la parcelle 6 de Paracou (1,65ha).
```{r}
#| label: SACP6Fig
#| echo: false
#| results: 'hide'
#| ref.label: 'SACCode'
#| fig.cap: "(ref:SACP6Fig)"
```


Code R pour réaliser la figure \@ref(fig:SACP6Fig):
```{r}
#| label: SACP6Code
#| eval: FALSE
paracou_6_abd %>% 
  filter(site == "subplot_1") %>% 
  accum_hill(q = 0) %>% 
  autoplot()
```

Plus l'ordre de la diversité est faible, plus l'importance des espèces rares dans son évalutation est grande.
La richesse est donc la mesure la plus sensible au biais d'échantillonnage.
Mais c'est aussi la mesure la plus simple et la plus utilisée: elle a donc été largement étudiée et bénéficie d'une large gamme de techniques d'estimations.

Les méthodes les plus utilisées sont dites non paramétriques.
Elles ne font pas de supposition sur la forme de la courbe d'accumulation des espèces ni sur la distribution des abondances
Les estimateurs les plus connus sont ceux de @Chao1984 et le *jackknife* [@Burnham1979].

Une alternative consiste à inférer à partir des données les paramètres d'une distribution d'abondance choisie.
...
Enfin, l'extrapolation de la courbe d'accumulation a été proposée par quelques auteurs.
...
L'estimation du nombre d'espèces non observées passe par une représentation géométrique.
Dans un espace à $S$ dimensions, le carré de la norme du vecteur dont les coordonnées sont $(1 - p_s)^{n / 2}$ est 
$$\sum_s{(1 - p_s)^n},$$
...
Cet estimateur est bien moins répandu que les précédents, notamment à cause de la nécessité de choisir $\kappa$ sans base théorique.
...
## Entropie de Shannon {#sec-BiaisShannon}
...
# Pratique de l'estimation de la diversité
...
Un nombre équivalent d'espèces existe pour toutes ces mesures.
Il est toujours égal à l'inverse de l'image de l'entropie par la réciproque de la fonction d'information:
...
L'indice de Hurlbert, qui n'est pas une entropie, et l'entropie de Simpson généralisées sont présentées en détail ici.
...
## Entropie de Simpson généralisée
...
#| label: Paracou618S
paracou_6_abd %>% 
  dplyr::filter(site %in% paste("subplot", c(1,2), sep = "_")) ->
  p_6_12_abd
# Abondance
p_6_12_abd %>% 
  abd_sum() %>% 
...
```{r}
#| label: comProfilzetaFig
#| echo: false
#| results: 'hide'
#| message: 'false'
#| ref.label: 'comProfilzetaCode'
#| fig.cap: "(ref:comProfilzetaFig)"
#| out.width: '100%'
#| fig.asp: 0.5
```
...
```{r}
#| label: comProfilzetaCode
#| eval: false
# Carré n°2 de BCI
BCI_1ha_abd <- as.numeric(BCI[2, ])
# Hectare sud-ouest de Paracou #6. 
# dbmss traite la sélection dans un wmppp
library("dbmss")
# Arbres dans la fenêtre de coordonnées (0:100, 0:100)
paracou_6_wmppp[owin(c(0,100), c(0,100))] %>% 
  # Abondances
  as_abundances() %>% 
  # Transformation en vecteur
  as.numeric() -> p6_1ha_abd
# Abondances
BCI_1ha_n <- sum(BCI_1ha_abd)
p6_1ha_n <- sum(p6_1ha_abd)
# Richesse
BCI_1ha_S <- div_richness(BCI_1ha_abd, as_numeric = TRUE)
p6_1ha_S <- div_richness(p6_1ha_abd, as_numeric = TRUE)
# Préparation d'un tibble pour stocker les résultats
expand.grid(Forest = c("BCI", "Paracou"), order = seq_len(min(BCI_1ha_S, p6_1ha_S) - 1)) %>% 
  mutate(
    n = ifelse(Forest == "BCI", BCI_1ha_n, p6_1ha_n),
    GenSimp.z = NA, 
    GenSimp.sd = NA
  ) -> p_6_12_zeta
# Calcul de l'entropie
# Une boucle est nécessaire : les fonctions ne sont pas vectorielles
library("EntropyEstimation")
for (i in 1:nrow(p_6_12_zeta)) {
  abd <- (if (p_6_12_zeta[i, "Forest"] == "BCI") BCI_1ha_abd else p6_1ha_abd)
  order <- p_6_12_zeta[i, "order"]
  p_6_12_zeta[i, "GenSimp.z"] <- GenSimp.z(x = abd, r = order)
  p_6_12_zeta[i, "GenSimp.sd"] <- GenSimp.sd(x = abd, r = order)
}
...
p_6_12_zeta %>% 
  mutate(
    ...
    ci = qnorm(1 - alpha / 2) * GenSimp.sd / sqrt(n),
    ent_inf = GenSimp.z - ci,
    ent_sup = GenSimp.z + ci,
    # Transformation de l'entropie en diversité
    Diversity = 1 / (1 - GenSimp.z^(1 / order)),
    div_inf =  1 / (1 - ent_inf^(1 / order)),
    div_sup = 1 / (1 - ent_sup^(1 / order))
  ) %>% 
  ggplot(aes(x = order, color = Forest)) +
    geom_ribbon(aes(ymin = div_inf, ymax = div_sup), alpha = 0.3) +
    geom_line(aes(y = Diversity)) +
    labs(
      x = "Ordre de diversité", 
...
n_simulations <- 100
profile_hill(BCI_1ha_abd, n_simulations = n_simulations, alpha = alpha) %>% 
  mutate(Forest = "BCI") %>% 
  # Deuxième profil ajouté au tibble
  bind_rows(
    profile_hill(p6_1ha_abd, n_simulations = n_simulations, alpha = alpha) %>% 
...
#| label: zetaDiffFig
#| echo: false
#| eval: false # TODO: corriger et activer
#| results: 'hide'
#| message: 'false'
#| ref.label: 'zetaDiffCode'
#| fig.cap: "(ref:zetaDiffFig)"
#| out.width: '100%'
#| fig.asp: 0.5
```
...
## Entropie généralisée des mesures d'inégalité
TODO: déplacer dans décomposition-autres approches.
...
\begin{equation}
  (\#eq:HqEco)
  H_q = 
    \frac{1}{q\left(q - 1\right)} 
    \left[\frac{1}{S} \sum^S_{s=1}{\left(\frac{n_s}{\bar{n_s}}\right)^{q}} - 1\right].
\end{equation}
