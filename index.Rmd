...
url: https://github.com/EricMarcon/MesuresBioDiv3/
github-repo: EricMarcon/MesuresBioDiv3
...
# Chunk font size hook: 
# allows size='small' or any valid Latex font size in chunk options
...
packages <- c("ade4", "adespatial", "cowplot", "dbmss", "entropy", 
...
# GitHub packages
if (!"divent" %in% installed.packages()[, 1]) {
...
La question n'est pas épuisée, et fait toujours l'objet d'une recherche active et de controverses [@Ricotta2005b].

Le document est organisé en 7 parties.
En guise d'introduction, la première partie présente les *notions de diversité*, suivie des différentes approches permettant de concrétiser ces notions sous forme de mesures:

- La biodiversité neutre, qui ne prend pas en compte les relations entre les espèces.
C'est l'approche la plus commune, qui inclut par exemple la richesse spécifique et les indices de Shannon et Simpson.
La théorie de l'information permet de généraliser les indices historiques dans un cadre robuste aboutissant à la définition des nombres effectifs d'espèces, aussi appelés nombres de @Hill1973
- Les diversités phylogénétique et fonctionnelle, qui incluent les relations (généralement, les distances) entre espèces.
Leur objectif est de prendre en compte l'histoire évolutive des espèces ou, dans une démarche assez différente, de considérer leur niche plutôt que leur identité.
- La décomposition de la diversité selon plusieurs niveaux hiérarchisés.
Cette approche permet à la fois les changements d'échelle, à travers les notions de diversité $\alpha$ et $\gamma$, et de définir deux nouvelles notions de diversité (classiquement regroupée sous le terme diversité $\beta$): la diversité de différentiation et la diversité de partitionnement.
- La diversité jointe qui prend en compte en même temps la diversité des espèces dans chaque habitat et la diversité des habitats occupés par chaque espèce.
- La diversité spatialement explicite qui permet de clarifier les notions d'accumulation de diversité (dépendant de l'échantillonnage) et de diversité régionale, mais aussi de spatialiser les mesures de biodiversité en lien avec les questions d'écologie.
- Les estimations de diversité à large échelle, souvent réduite à la richesse spécifique, qui repose sur une large palette de méthodes.
...
Les calculs sont réalisés dans R [@R], essentiellement avec le package *divent* [@R-divent], qui succède au package *entropart* [@Marcon2014c]. 
L'ensemble du code est disponible sur GitHub^[<https://github.com/EricMarcon/MesuresBioDiv3/>] où se trouvent les mises à jour de ce document^[<https://ericmarcon.github.io/MesuresBioDiv3/>].
...
#| fig.env: 'figure'
#| out.width:  !expr ifelse(knitr::is_latex_output(), '\\widthw', '120%')
...
$f_1$ s'appelle le nombre de *singletons* et $f_2$ le nombre de *doubletons.*
...
Son estimateur le plus simple, ${\hat{p}}_s = r_s$ est la fréquence observée.
...
\noindent $r_s$: la fréquence de l'espèce $s$ dans un échantillon.
